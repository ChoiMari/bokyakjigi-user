...
public class AuthService {
     ...
     * @return SignInResponseDto 반환 (accessToken, refreshToken, tokenType 포함)
    ...
    public SignInResponseDto signIn(String email, String rawPassword) {
        log.info("signIn start (email={})",email);
        // 이메일과 탈퇴 여부 'N'로 활성 회원 조회
        Member member = memberRepository.findByEmailAndIsDeleted(email, IsDeleted.N) //email이 일치하고, isDeleted가 N인 MEMBER객체를 찾음
